...
def inst_all(f_inst,exts,pth='.'):
  global f_lst
  global extension
  f_inst.write('''Section "Files (required)"\n''')
  f_inst.write('''SectionIn RO\n''')
  for i in exts:
    extension=i
    for s in filter(str_iter, f_lst):
      f_inst.write('''  SetOutPath $INSTDIR\\''')
...
def uninst_all(f_inst,exts,pth='.'):
  global f_lst
  global extension
  f_inst.write('''Section "un.Files"\n''')
  for i in exts:
    extension=i
    for s in filter(str_iter, f_lst):
      f_inst.write('''  Delete $INSTDIR\\''')
