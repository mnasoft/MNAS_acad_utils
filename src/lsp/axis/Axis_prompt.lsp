...
(setq mnas-axis:multiple-graph-prompt
       "Построение семейства полилиний в координатах двух шкал
Задаются:
1) str-x - строка, содержащая имя оси Х, с которой дложны быть связаны значения по оси Х;
2) str-y-n - строка, которая содержит имена переменных на осях Y1...YN, с которыми связаны данные по этим осям.
...
В ответ на приглашение нужно вводить TAU и Y.
Чтобы преобразовать отрезок в ось нужно воспользоваться командой mnas-axis-edit."
       )
