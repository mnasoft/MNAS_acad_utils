...
;;;;;; 11) AT	 - размораживать все слои;\n
;;;;;; 12) AU	 - разблокировать все слои;\n
;;;;;; 13) Exit - выходить из команды управления слоями."

;;;;;;("Set" "Позволяет устанавливать текущий слой" "Слои")
(defun c:Set  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (progn (setq la (cdr (assoc 8 (entget (ssname la_l 0))))) (command "_.layer" "_set" la ""))))

;;;;;;("Off" "Позволяет выключать отмеченные слои" "Слои")
(defun c:Off  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (if	(setq la (layer_cl la_l (getvar "clayer")))
...
;;;;;;("Wo" "Позволяет выключать все слои за исключением отмеченных" "Слои")
(defun c:Wo  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (command "_.layer"
	     ...
	     (layer_list la_l)
	     "")))
;;;;;;("Un" "Позволяет разблокировать отмеченные слои" "Слои")
(defun c:Un  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (command "_.layer" "_Unlock" (layer_list la_l) "")))

;;;;;;("Lo" "Позволяет блокировать отмеченные слои" "Слои")
(defun c:Lo  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (command "_.layer" "_lock" (layer_list la_l) "")))

;;;;;;("Col" "Позволяет задавать цвет для отмеченных слоев" "Слои")
(defun c:Col  (/ la_l)
  (setq la_l (ssget))
  (setq col (getstring "\nEnter color : "))
  (if (and (/= col nil) la_l)
...
;;;;;;("Wl" "Позволяет блокировать все слои за исключением отмеченных" "Слои")
(defun c:Wl  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (command "_.layer"
...
;;;;;;("WFr" "Позволяет замораживать все слои за исключением отмеченных" "Слои")
(defun c:WFr  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (command "_.layer"
...
;;;;;;("Fr" "Позволяет замораживать отмеченные слои" "Слои")
(defun c:Fr  (/ la_l)
  (setq la_l (ssget))
  (if la_l
    (if	(setq la (layer_cl la_l (getvar "clayer")))
...
;;;;;;("Ao" "Позволяет включать все слои" "Слои")
(defun c:Ao () (command "_.layer" "_on" "~" ""))

;;;;;;("Au" "Позволяет разблокировать все слои" "Слои")
(defun c:Au () (command "_.layer" "_unlock" "~" ""))

;;;;;;("At" "Позволяет размораживать все слои" "Слои")
(defun c:At () (command "_.layer" "_Thaw" "~" ""))
