...
(defun t2  (lst_1 / temp) ;Возвращает список, состоящий из имен и типов
  (mapcar (function
	    (lambda (el) (setq temp (cons (list el (type_as_string (eval (read el)))) temp))))
	  lst_1)
  (reverse temp))
...
(defun setup_r	()
  ...
  (if flag_r
    ...
    (set_tile "t_1_1r" "И+Т"))
  (set_tile "t_1_2r"
	    (strcat (itoa (length str_atoms_r)) " из " (itoa (length all_atoms_r))))
...
(defun setup_l	()
  (if flag_l
    ...
    (set_tile "t_1_1l" "И+Т"))
  (set_tile "t_1_2l"
	    (strcat (itoa (length str_atoms_l)) " из " (itoa (length all_atoms_l))))
...
(defun c:atoms	(/ act dcl_id do_dial e_1_1 all_atoms_l str_atoms_l poz_list_l)
  (init)
  (setq dcl_id (load_dialog (findfile "acad_w.sup/atoms/atoms-dlg.dcl")))
  (if (< dcl_id 0)
    (exit))
