;;;(princ "\nЗагружаю CHANGE/CH_WID.lsp   ")
(princ (strcat "\nЗагружаю " (acad_sup) "/" "CHANGE/CH_WID.lsp   "))
...
(defun c:ch_wid	(/ li w nli i en ed ty p1 p2 cen rad as ae ps pe di do del_elem )
  ...
  (while (<= i nli)
    ...
    (cond
      ((= ty "LINE") (ch:ln_pl ed w) (setq del_elem t))
...
(defun ch:sp_pl	(ed wid / tmp pt en)
  (mapcar
	...
	(cond
	  ((= (car el) 10)
	   (setq tmp (cons (list (cadr el) (caddr el)) tmp))
	  )
	)
	t
      )
...
(defun ch:ar_pl	(ed wid / cen rad as ae ps pe)
  ...
  (command
    "_.pline" "_non" ps	"_w" wid wid "_a" "_ce"	"_non" cen "_non" pe "")
  (command "matchprop" (dxf-get -1 ed) (entlast) "")
)
...
(defun ch:lw_pl	(en wid)
  (command "_.pedit" en "_w" wid "_x")
)
