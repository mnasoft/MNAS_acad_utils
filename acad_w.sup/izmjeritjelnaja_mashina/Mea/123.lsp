...
;;;  (setq fn (getfiled "title" "default" "ijk" 0))
  (setq	fn (findfile
	     (strcat (acad_sup) "\\Измерительная машина\\Mea\\h10_1.ijk")
	   )
  )
  (if (null fn)
    (progn
      (alert
	(princ
	  (strcat "Не могу открыть файл :" (strcat (acad_sup) "\\Измерительная машина\\Mea\\h10_1.ijk"))
	)
      )
      (exit)
    )
...
(defun xyz_ijk->xyz(xyz_ijk)
  (list (car xyz_ijk) (cadr xyz_ijk) (caddr xyz_ijk))
)
...
(defun draw_xyz	(xyz_ijk_lst color)
  (mapcar
    (function
      (lambda (xyz_ijk)
	(dr:point (list (car xyz_ijk) (cadr xyz_ijk) (caddr xyz_ijk)) 0)
      )
    )
    xyz_ijk_lst
  )
...
(defun draw_ijk	(xyz_ijk_lst scale color)
  (mapcar
    (function
      (lambda (xyz_ijk / xyz ijk xyz+ijk*scale)
...
(120 2 20 2 nil "end of" 100 20 0 0 0 T T nil T)
