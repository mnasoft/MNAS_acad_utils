;;;(princ "\nЗагружаю spec/func_3 ")
(princ (strcat "\nЗагружаю " (acad_sup) "/" "spec/func_3 "))
...
(defun setup_l_3_1 ()
  ...
  (mapcar
	...
	(add_list
	  (apply
	    'strcat
	    (mapcar (function (lambda (e1) (strcat (cdr e1) "\t"))) e)
	  )
	)
      )
    )
...
(defun ac_b_3_4	()
  ...
  (mapcar
    (function (lambda (e)
	 ...
	 dlg_1_kv
	  (subst
	    (cons (cdr e) (cdr (assoc (car e) sp_curr)))
	    (assoc (cdr e) dlg_1_kv)
	    dlg_1_kv
	  )
       )
       t
...
(defun ac_dlg_3	($key $value)
  (setq	dlg_3_kv
	 (subst (cons $key $value) (assoc $key dlg_3_kv) dlg_3_kv)
  )
)
...
(defun setup_f3_ac ()
  ...
  (mapcar
    (function
      (lambda (e) (action_tile (car e) "(ac_dlg_3 $key $value)"))
    )
    dlg_3_kv
  )
)
...
(defun setup_f3_dialog (/ temp)
  ...
  (setq	sp_curr
	 (mapcar (function (lambda (e)
		    (cons (car e) (cdr (assoc (cdr e) dlg_1_kv)))
		  ))
		 d1_sp_curr
	 )
  )
  (cond
    (
     (= action_3 2)
     (setq temp (get_poz "l_3_1"))
     (ins_befor 'sp_curr 'temp 'sp_poz)
    )
    (
     (= action_3 3)
     (setq temp (get_poz "l_3_1"))
     (ins_after 'sp_curr 'temp 'sp_poz)
    )
    (
     (= action_3 5)
     (setq temp (get_poz "l_3_1"))
