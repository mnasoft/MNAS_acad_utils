...
(defun бд_ac_l_5_1(val)
 (заполнить_формат...описание (nth (atoi val) бд_наименование))
)

(defun заполнить_формат...описание(список)
  (set_tile "e_5_6" (cdr(assoc 3 список )));;;Формат
  (set_tile "e_5_7" (cdr(assoc 1 список )));;;Обозначение
  (set_tile "e_5_8" (cdr(assoc 2 список )));;;Наименование
  (set_tile "e_5_9" (cdr(assoc 4 список )));;;Материал
  (set_tile "e_5_10" (cdr(assoc 5 список )));;;Примечание
  (set_tile "e_5_11" (cdr(assoc 6 список )));;;Описание
)
...
(defun бд_setup_dialog (/ str_in_l_box)
  (load (strcat (acad_sup) "/spec/bd_naimjenovanije.lsp"))
  (setq
    str_in_l_box
     (mapcar
       (function
	 (lambda (el)
	   ...
	   (strcat формат "\t" обознач. "\t" наим. "\t" материал "\t" примечание "\t" описание)
	 )
       )
       бд_наименование
     )
...
(defun бд_от_расшифровки_к_базе_наименование()
  (setq
    fn
     (getfiled "Открытие файла для добавления в базу наименование"
	       ""
	       "txt"
	       0
     )
  ...
  (while (setq str (read-line fl ) )
    ...
    (setq
	 ...
	 (cons 6 "")
      ...
      бд_наименование (append бд_наименование (list запись_бд_наименование))
    )
  )
  (close fl)
)	
(defun бд_от_расшифровки_к_базе_спецификация()
  t
)
