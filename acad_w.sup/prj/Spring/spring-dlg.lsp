...
(setq setup_msg '(("d" "Диаметр проволоки, мм")
...
(defun stp_lst () (setq setup_lst (action-save_dlg setup_lst)))
...
(defun action-tile_dlg-spring  (@setup_lst @setup_lst_name)
  (mapcar (function (lambda (el)
                      (action_tile
                        (car el)
                        (strcat "(err)" "(setq " @setup_lst_name " (action-save_dlg " @setup_lst "))"))))
          (eval (read @setup_lst))))

(action-tile_dlg "setup_lst" "setup_lst")

(defun about () (alert (strcat "Расчет пружины сжатия " (about-gpl-string))))
