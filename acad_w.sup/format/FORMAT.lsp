;;;(princ "\nЗагружаю FORMAT/format       ")
(princ (strcat "\nЗагружаю " (acad_sup) "/" "FORMAT/format       "))
...
(defun c:format	(/ ;
  ...
  (setq	format_registry
	 '((p_start (0.0 0.0 0.0))
	   (sht1_val
	    ("Устройство горелочное"	       "В2М80009993СБ"
	     "Дораб.черт. Н80038002СБ"	       ""
	     ""		      ""	       ""
	     "1:1"	      ""	       "1"
	     "ЖАКИ"	      "Нач отд"	       "Матвеев   "
	     "Матвеев   "     "Дунаев   "      "Ванцовский"
	     "Матвеев   "     "Спицын   "      ""
	     ""
	    )
	   )
...
(defun load_format ()
  ...
  (setq f_key (list "1" "2" "2аг" "2ат" "2бн" "2бч" "3")) ;Тип штампа

  (setq	f_val
	 (list
	   "1   - чертежи и схемы    і  1    "		      "2   - текст.констр.док.  і  1    "
	   "2аг - четрежи и схемы    і  n    "		      "2ат - текст.констр.док.  і  n    "
	   "2бн - текст.констр.док.  і(2*n+1)"		      "2бч - текст.констр.док.  і (2*n) "
	   "3   - только рамка       і  ЛУ   "
	  )
  )
