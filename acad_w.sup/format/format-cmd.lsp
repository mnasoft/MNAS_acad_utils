...
(defun c:format	 (/ ;
		  ...
		  f_val
		  ;;Список, содержащий полные наименования основных надписей.
		  sht1_key
		  ;;		Ключи  тегов в основной надписи
		  sht1_key2
		  ;;		Наименования тегов в основной надписи
		  kr_key
		  ;; Список, содержащий допустимые кратности для форматов (строки).
		  kr_val
		  ;;		Список, содержащий допустимые кратности для форматов (целочисленные).
		  for_name ;		Список, содержащий имена форматов.
		  for_val ;		Список, содержащий размеры форматов.
		  reg_root
		  ;;		Корнь реестра для записи переменных параметров.
		  format_registry
		  ;;	Список, содержащий значения параметров диалога по умолчанию.
		  f_no
		  ;;			Индекс основной надписи.
		  kr_no
		  ;;			Индекс для нахождения кратности формата.
     ;
		  )
  ...
  (setq	format_registry	'((p_start (0.0 0.0 0.0))
			  (sht1_val
			   ("Устройство горелочное"
			    "В2М80009993СБ"  "Дораб.черт. Н80038002СБ"
			    ""		     ""
			    ""		     ""
			    "1:1"	     ""
			    "1"		     "ЖАКИ"
...
(defun raz  (/ for)
  (if (and (/= for_n nil) (/= kr_n nil))
    (progn (setq for (list (nth 0 (nth for_n for_val)) (* (nth 1 (nth for_n for_val)) (nth kr_n kr_val))))
	   ...
	   (if (= dir_sh 0)
	     (setq for (reverse for)))
	   (set_tile "razmer" (strcat (itoa (nth 0 for)) " x " (itoa (nth 1 for))))) ;progn
    ))
