	  ...
	  ((= i1 2)
	   (if (or (and (= dir_sht 1) (> (nth kr_no kr_val) 1))
	     ...
	     (setq p0 (polar p_start (/ pi 2.0) (nth 1 for))
		   ...
		   p3 (mapcar (function +) p3 p0)
		   p4 (mapcar (function +) p4 p0))
	     (setq p0  (polar (polar (mapcar (function +) p_start '(0.0 15.0 0.0)) 0.0 (nth 0 for))
			      (/ pi 2.)
			      (nth 1 for))
		   an0 "-90.0d"
...
(defun draw_zona  (/ kr ff ff1 en1)
  (setq	pick2 (ssadd)
	en1   (entlast))
  (draw_line (mapcar (function +) p_start (list 15.0 5.0 0.0))
  ...
  (draw_line (mapcar (function +) p_start (list 15.0 5.0 0.0))
	     ...
	     7)
  (draw_line (mapcar (function +) p_start (list 20.0 (- (cadr for) 5.0) 0.0))
	     (mapcar (function +) p_start (list 15.0 (- (cadr for) 5.0) 0.0))
	     7)
  ...
  (setq l1 (nth 1 ff))
  (while (> i1 0)
    (draw_line (mapcar (function +) (list 0.0 (* l1 i1) 0.0) p_start)
  ...
  (setq l1 (nth 0 ff))
  (while (> i1 0)
    (draw_line (mapcar (function +) (list (* l1 i1) 0.0 0.0) p_start)
