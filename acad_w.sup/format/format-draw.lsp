	  ...
	  ((= i1 2)
	   (if (or (and (= dir_sht 1) (> (nth kr_no kr_val) 1))
	     ...
	     (setq p0 (polar p_start (/ pi 2.0) (nth 1 for))
		   ...
		   p3 (mapcar '+ p3 p0)
		   p4 (mapcar '+ p4 p0))
	     (setq p0  (polar (polar (mapcar '+ p_start '(0.0 15.0 0.0)) 0.0 (nth 0 for))
			      (/ pi 2.)
			      (nth 1 for))
		   an0 "-90.0d"
		   p1  (polar (polar (pp_a p_start p1 (/ pi -2.0)) 0.0 (nth 0 for))
			      (/ pi 2.0)
			      (+ (nth 1 for) 15.))
		   p2  (polar (polar (pp_a p_start p2 (/ pi -2.0)) 0.0 (nth 0 for))
			      (/ pi 2.0)
			      (+ (nth 1 for) 15.))
		   p3  (polar (polar (pp_a p_start p3 (/ pi -2.0)) 0.0 (nth 0 for))
			      (/ pi 2.0)
			      (+ (nth 1 for) 15.))
		   p4  (polar (polar (pp_a p_start p4 (/ pi -2.0)) 0.0 (nth 0 for))
			      (/ pi 2.0)
			      (+ (nth 1 for) 15.))))))
    (command "_insert"
...
(defun draw_zona  (/ kr ff ff1 en1)
  ...
  (setq l1 (nth 1 ff))
  (while (> i1 0)
    (draw_line (mapcar '+ (list 0.0 (* l1 i1) 0.0) p_start)
  ...
  (setq l1 (nth 0 ff))
  (while (> i1 0)
    (draw_line (mapcar '+ (list (* l1 i1) 0.0 0.0) p_start)
  ...
  (while (> i1 0)
    (if	(= 1 i1)
      (draw_text (mapcar '+ p_start (list 17.5 168.5 0.0)) "T" (chr (+ zone_ch i1 -1)))
      (draw_text
	(mapcar '+ (list 0.0 (* (nth 1 ff) i1) 0.0) p_start (list 17.5 (/ (nth 1 ff) -2.) 0.0))
	"T"
	(chr (+ zone_ch i1 -1))))
    (draw_text (mapcar '+
	       ...
	       (chr (+ zone_ch i1 -1)))
    (setq i1 (1- i1)))
  (hid_re (mapcar '+ p_start '(20.0 5.0 0.0))
	  (mapcar '+ '(-5.0 5.0 0.0) (polar p_start 0.0 (nth 0 for)))
	  (mapcar '+ p_start '(-5.0 -5.0 0.0) for)
	  (mapcar '+ '(20 -5.0 0.0) (polar p_start (/ pi 2.) (nth 1 for)))
	  pick2))
...
(defun draw_shtamp  (/ s1)
  ...
  (cond
    ((= s1 "1")
	 ...
	 (strcat "*" (extend_path "/FORMAT/sht_2"))
	 "_non"
	 (polar (polar (mapcar '+ p_start '(0.0 15.0 0.0)) 0.0 (nth 0 for)) (/ pi 2.) (nth 1 for))
	 "1"
	 "-90"))
    ...
    ((= s1 "2бн")
     (command "_insert"
	      ...
	      "0"))
    ((= s1 "2бч")
     (command "_insert" (strcat "*" (extend_path "/FORMAT/SHT_6_L")) "_non" p_start "1" "0"))
    ((= s1 "3"))))
