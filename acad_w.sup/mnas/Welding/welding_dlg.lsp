(defun c:edweld	(/								;
  ...
  (setq
    setup_lst	 (list
		   ...
		   (list tile_wedl_standard
			 "0"
										;(list "Тип А" "Тип Б" "Тип В" "Тип Г" "ЭЛС" "ГОСТ 5264" "ГОСТ 11534" "ГОСТ 14771-76" "ГОСТ 23518" "ГОСТ 15878")
			 (list "Тип А"		 "ГОСТ 5264-80"	   "Тип Б"	     "ГОСТ 11534-75"   "Тип В"
			       "ГОСТ 14771-76"	 "Тип Г"	   "ГОСТ 23518-79"   "Тип Д"	       "ГОСТ 15878-79"
			       "Тип Е"		 "ГОСТ 16037-80"   "ЭШС"	     "ГОСТ 15164-78"   "ЭЛС"
			       "И 255.105.111"
			      )
		   )
  ...
  ;; Загрузка диалога.
  (load_dcl "/MNAS/welding/welding.dcl")
  ...
  (while do_dialog
    ...
    (cond
	 ...
	 (function
	   (lambda (el)
	     (setq setup_lst (subst (list el "0")
				    (assoc el setup_lst)
				    setup_lst
			     )
	     )
	 ...
	 (function
	   (lambda (el)
	     (setq setup_lst (subst (list el "1")
				    (assoc el setup_lst)
				    setup_lst
			     )
	     )
...
(defun check_type ()
  (if (/= (cdr (assoc 0 ed))
    ...
    (progn
      (alert
	"Выбранный вами объект не соответствует типу MNASWelding."
      )
      (exit)
    )
  )
...
(130 2 80 2 nil "end of" 100 15 0 0 0 T T nil T)
