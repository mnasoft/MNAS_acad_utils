...
(defun c:pozition_set_text_scale (/ reg_root setup_lst reg_val text_scale line-offset ed text)
  ...
  (setq
    text	     (cadr (assoc "text" setup_lst))
  ...
  (setq
    ed (subst (cons 1 text) (assoc 1 ed) ed)
...
(defun c:edpos (/								;
  ...
  (load_dcl "/MNAS/pozition/pozition.dcl")
  ...
  (while do_dialog
    ...
    (cond
	 ...
	 (function
	   (lambda (el)
	     (setq setup_lst (subst (list el "0" (last (assoc el setup_lst)))
				    (assoc el setup_lst)
				    setup_lst
			     )
	     )
	 ...
	 (function
	   (lambda (el)
	     (setq setup_lst (subst (list el "1" (last (assoc el setup_lst)))
				    (assoc el setup_lst)
				    setup_lst
			     )
	     )
	 ...
	 '("text_toggle" "arrow_type_toggle" "dim_style_toggle"	"text_color_toggle" "text-scale_toggle"	"line-offset_toggle")
      ...
      ((= action 5)								;"text_color_image" "text_color"
       (setq
	 text_color_n (acad_colordlg (atoi (cadr (assoc "text_color_image" setup_lst))))
...
(defun ac_apply_btn ()
  (setq dia_pos_pos (done_dialog 2))
)
...
(defun ac_apply_do ()
  (setq
    ...
    ed		 (subst	(cons 91 (atoi (cadr (assoc "arrow_type" setup_lst))))
			(assoc 91 ed)
			ed
		 )
    ed		 (subst	(cons 1 (cadr (assoc "text" setup_lst)))
			(assoc 1 ed)
			ed
		 )
    ed		 (subst	(cons 40 (ATOF (cadr (assoc "text-scale" setup_lst))))
			(assoc 40 ed)
			ed
		 )
    ed		 (subst	(cons 41 (ATOF (cadr (assoc "line-offset" setup_lst))))
			(assoc 41 ed)
			ed
		 )
    ed		 (subst	(cons 70 (ATOI (cadr (assoc "text_color_image" setup_lst))))
			(assoc 70 ed)
			ed
		 )
  )
...
(defun check_type ()
  (if (/= (cdr (assoc 0 ed))
    ...
    (progn
      (alert "Выбранный вами объект не соответствует типу MNASPozition.")
      (exit)
    )
  )
)
...
(defun init_dlg_key (lst_setup / key)
  (mapcar
	...
	(cond
	  (
	   (is_equal_one_in_list key '("toggle" "edit_box"))
...
(defun action-tile_dlg_key (@setup_lst @setup_lst_name / rez)
  (mapcar
    (function
      (lambda (el)
	(action_tile (car el)
		     (strcat "(setq " @setup_lst_name " (action-save_dlg_key " @setup_lst "))")
	)
      )
    )
    (eval (read @setup_lst))
...
(defun action-save_dlg_key (setup_lst / key)
  ...
  (mapcar
    (function
      (lambda (el / el_last el_post_pros)
	(setq
	  el_last      (last el)
...
(defun default_settings	()
  (setq
    setup_lst
	      (subst
		(list "text-scale" (rtos (sqrt 2.0)) (list "edit_box"))
...
(defun select-init ()
  ...
  (setq
    arrow_type_tile (assoc "arrow_type" setup_lst)
    arrow_type_lst  (caddr arrow_type_tile)
    arrow_type_n    (cdr (assoc 91 ed))
    setup_lst	    (subst
		      (list "arrow_type" (rtos arrow_type_n) arrow_type_lst (last (assoc "arrow_type" setup_lst)))
		      arrow_type_tile
		      setup_lst
		    )
  )
  ...
  (setq
    text-scale_tile (assoc "text-scale" setup_lst)
    text-scale_n    (cdr (assoc 40 ed))
    setup_lst	    (subst
		      (list "text-scale" (rtos text-scale_n) (last (assoc "text-scale" setup_lst)))
		      text-scale_tile
		      setup_lst
		    )
  )
  ...
  (setq
    text_tile (assoc "text" setup_lst)
    text_n    (cdr (assoc 1 ed))
    setup_lst (subst
		(list "text" text_n (last (assoc "text" setup_lst)))
		text_tile
		setup_lst
	      )
  )
  ...
  (setq
    text_color_image_tile (assoc "text_color_image" setup_lst)
    text_color_image_n	  (cdr (assoc 70 ed))
    setup_lst		  (subst
			    (list "text_color_image" (itoa text_color_image_n) (last (assoc "text_color_image" setup_lst)))
  ...
  (setq
    line-offset_tile (assoc "line-offset" setup_lst)
    line-offset_n    (cdr (assoc 41 ed))
    setup_lst	     (subst
		       (list "line-offset" (rtos line-offset_n) (last (assoc "line-offset" setup_lst)))
		       line-offset_tile
		       setup_lst
		     )
  )
...
(defun ac_text_scale_+ ()
  ...
  (setq
    setup_lst
     (subst
       (list "text-scale" (get_tile "text-scale") (list "edit_box"))
       (assoc "text-scale" setup_lst)
       setup_lst
     )
  )
