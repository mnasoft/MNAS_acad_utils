...
(defun spusk (f
     ;	- имя функции, максимальное значение которой отыскивается 
	      x ;	- список параметров в текущей точке
	      h
     ;	- список приращений для шагов по каждому из параметров
	      k_u
     ;	- список коэффициентов усиления для каждого из приращений (дб > 1.0)
	      k_o
     ;	- список коэффициентов ослабления для каждого из приращений (дб < 1.0)
	      ...
	      iter ;	- итерация
	      i
     ;	- номер координаты по которой производится спуск (изменяется от 0 до n-1)
	      n ;	- количество параметров
	      x+
     ;	- функция производит расчет x_temp при шаге из x по i-тому папараметру в +
	      x-
     ;	- функция производит расчет x_temp при шаге из x по i-тому папараметру в -
	      apl
     ;	- выполняет нахождение зачения функции f при параметрах x_temp
  ...
  (if (null red_list_lsp)
    (load (strcat (acad_sup) "/UTILS/red_list.lsp"))
  )
