...
(defun dr:line (pt1 pt2 col / line)
  ...
  (if (and (>= col 0) (<= col 256))(vla-put-color line col))
  line
)
...
(defun dr:xline (pt1 pt2 col / xline)
  ...
  (if (and (>= col 0) (<= col 256))(vla-put-color xline col))
  xline
)
...
(defun dr:ray (pt1 pt2 col / ray)
 (setq ray (vla-AddRay (AcadAppDocSpace) (vlax-3d-point pt1) (vlax-3d-point pt2)))
  (if (and (>= col 0) (<= col 256))(vla-put-color ray col))
  ray
)
...
(defun dr:text (txt pt height rotation col / text)
  ...
  (if (and (>= col 0) (<= col 256))(vla-put-color text col))
...
(defun dr:solid (point1 point2 point3 point4 color / solid)
 ...
 (if (and (>= color 0) (<= color 256))(vla-put-color solid color))
 solid
)
...
(defun dr:spline(pts col / arr len spline)
  ...
  (if (and (>= col 0) (<= col 256))(vla-put-color spline col))
  spline
)
