...
(defun prinf (str var f) (princ (strcat "(setq " str " '") f) (prin1 var f) (princ ")\n" f))
...
(defun ins_befor  ($obj $no $li / $i $temp)
  (set $no
       (cond ((< (eval $no) 0) 0)
             ((and (>= (eval $no) 0) (<= (eval $no) (length (eval $li)))) (eval $no))
...
(defun del  ($no $li / $i $temp)
  ...
  (set $no
       (cond ((< (eval $no) 0) 0)
             ((and (> (eval $no) 0) (< (eval $no) (length (eval $li)))) (1- (eval $no)))
             ((>= (eval $no) (length (eval $li))) (length (eval $li)))
             ((null $temp) nil)
             (t 0)))
  (set $li (reverse $temp)))
