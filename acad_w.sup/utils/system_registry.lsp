...
(defun reg_read_default_lst (root lst)
  (mapcar
    (function
      (lambda (el)
	(reg_read_default root (car el) (cadr el))
      )
    )
    lst
  )
...
(defun reg_write_default_lst (root lst)
  (mapcar
    (function
      (lambda (el)
	(reg_write_default root (car el) (cadr el))
      )
    )
    lst
  )
