;;;(princ "\nЗагружаю D_chprop/D_chprop   ")
(princ (strcat "\nЗагружаю " (acad_sup) "/" "D_chprop/D_chprop   ") )
(vl-arx-import 'acad_colordlg)


;;;;;;("dch" "Программа управления свойствами объектов." "Измени")
(defun c:dch (/		*error*	  action    by_block  by_layer
	      color1	color4	  dcl_id    do_dialog ename1
	      err	layer1	  layer4    ltype1    ltype4
	      old_err
	     )
  (setq	old_err	*error*
	*error*	err
  ...
  (setq
    dcl_id (load_dialog (strcat (acad_sup) "/d_chprop/d_chprop.dcl"))
  )
  (if (< dcl_id 0)
    (exit)
  )
...
(defun ac-4 (/ q dcl_id)
  ...
  (while (setq layer2 (tblnext "layer"))
    (setq layer3 (cons layer2 layer3))
  )
...
(defun ac-5 (/ q dcl_id)
  ...
  (while (setq ltype2 (tblnext "ltype"))
    (setq ltype3 (cons ltype2 ltype3))
  )
...
(defun bc_4 (value)
  (set_tile "b7" (setq ltype3 (nth (atoi value) ltype2)))
)

(defun cc_4 (value)
  (set_tile "c5" (setq layer3 (nth (atoi value) layer2)))
)
