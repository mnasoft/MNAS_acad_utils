...
;;; не законЧена
  (setq
    pts	(list (assoc 10 sh:data) (assoc 11 sh:data))
    xdt	(list
	  (cons 1040 (assoc 40 sh:data))
...
(defun sc:pt_val (pt sc	/ sc_flag sc_p0	sc_p1 sc_v0 sc_v1 v_rez	p1-p0
		  a_max)
  (setq
    sc_p0   (cdr (assoc 10 sc))
    ...
    f_pr
	    (cond
	      ((= 0 sc_flag) sc:val_pt_porp)
	      ((= 1 sc_flag) sc:val_pt_log)
	    )
    f_obr
	    (cond
	      ((= 0 sc_flag) sc:pt_val_porp)
	      ((= 1 sc_flag) sc:pt_val_log)
	    )
  )
  (cond
    ((= a_max (car p1-p0)) (setq f_cord car))
  ...
  (f_obr
	...
	(/
	  (- (f_cord pt) (f_cord sc_p0))
	  (- (f_cord sc_p1) (f_cord sc_p0))
	)
      )
    )
  )
...
(defun sc:val_pt (vt sc / sc_p0 sc_p1 sc_v0 sc_v1 sc_flag v_factor p_rez)
  ...
  (mapcar
    (function
      (lambda (el0 el1) (+ el0 (* (- el1 el0) v_factor)))
    )
    sc_p0
    sc_p1
  )
...
(defun sc:pxy_pt(pxy scx scy / SCX_P0 SCX_P1 SCY_P0 SCY_P1 px py p_rez)
  (setq
    ...
    p_rez
	   (inters
	     px
	     (polar px (angle scy_p0 scy_p1) 1.0)
	     py
	     (polar py (angle scx_p0 scx_p1) 1.0)
	     nil
	   )
  )
  p_rez
  ...
  (setq
    ...
    px
	   (inters
	     ...
	     scx_p1
	     pt
	     (polar pt (angle scy_p0 scy_p1) 1.0)
	     nil
	   )
    py
	   (inters
	     ...
	     scy_p1
	     pt
	     (polar pt (angle scx_p0 scx_p1) 1.0)
	     nil
	   )
