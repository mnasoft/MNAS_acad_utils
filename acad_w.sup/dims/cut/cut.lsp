;;;(princ "\nЗагружаю dims/cut/cut        ")
(princ (strcat "\nЗагружаю " (acad_sup) "/" "dims/cut/cut        "))
...
;;;  (setq str (getstring "\nТекст вида:"))
  ...
  (while (cond
	   (pt_last (setq pt (getpoint pt_last "\nТочка:")))
	   (t (setq pt (getpoint "\nТочка:")))
	 )
    (if	(and pt pt_last)
...
(defun dr:arrow	(p_0 p_1 dang / p00 pl ang p_ar1 p_ar2 p_ar3 arr dir_text ;
  ...
  (setq
    ptxt     (polar p_00 ang len_tx1)
    ptxt     (polar ptxt (angle p_1 p_0) len_tx2)
    vid_text
	     (vla-AddText
	       acadSpace
	       str
...
(defun draw:cut	(pts	   str	     sc	       pt_dir	 /	   acadSpace
		 arr_len1  arr_len2  arr_len3  arr_wid1	 arr_wid2  i	     k	       lin_big
		 lin_small pl	     p_dr_k    p_dr_k+1	 p_dr_k-1  p_k	     p_k+1     p_k-1
		 txt_hi	   txt_1     txt_2     reactor	 txt_lst
		)
  (setq
    acadSpace  (AcadAppDocSpace); Пространство в котором следует рисовать примитивы
	 ...
	 dang (-
		(angle
		  (inters
		    ...
		    p_e0
		    pt_dir
		    (polar pt_dir (+ (* 0.5 pi) (angle p_e1 p_e0)) 1.0)
		    nil
		  )
		  pt_dir
		)
