...
(defun draw_sher  (p_top p_bas / bl_name d1 d2 d3 perev tb ed en ob)
  ...
  (cond	       ; Текст для значения шероховатости
    ((and (= (cadr (assoc "tgl_1_max_val" setup_lst)) "1")
     ...
     (setq
       tb (point_box
	    (append tb (draw_5 (+ d1 (* 2.0 d2)) d2 (cadr (assoc "eb_1_max_val" setup_lst)) perev))))
     (setq
       tb (point_box
	    (append tb (draw_5 (+ d1 (* 0.5 d2)) d2 (cadr (assoc "eb_1_min_val" setup_lst)) perev)))))
    ...
    ((and (= (cadr (assoc "tgl_1_max_val" setup_lst)) "1")
     ...
     (setq
       tb (point_box
	    (append tb (draw_5 (+ d1 (* 2.0 d2)) d2 (cadr (assoc "eb_1_min_val" setup_lst)) perev))))
     (setq
       tb (point_box
	    (append tb (draw_5 (+ d1 (* 0.5 d2)) d2 (cadr (assoc "eb_1_max_val" setup_lst)) perev)))))
