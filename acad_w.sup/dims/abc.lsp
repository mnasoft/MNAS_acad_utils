...
(defun index:++	(/ index_A index_0)
  (setq
    last_index (index:getlast)
    index_A    (car last_index)
    index_0    (cadr last_index)
  )
  (cond
    ((< index_A (index:abc_len)) (setq index_A (1+ index_A)))
...
(defun index:add ()
  (cond
    ((null index:list) (setq index:list (cons (index:getlast) index:list)) (index:++))
