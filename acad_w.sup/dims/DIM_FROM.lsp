(princ (strcat "\nЗагружаю " (acad_sup) "/" "dims/dim_from       "))

(vl-load-com)


;;;;;;("ins_blk" "Вставка блока с масштабом равным масштабу размеров." "Размеры")
(defun c:ins_blk (/ p0 p1 dir_0 name sc)
  (setq p0 (getpoint "\nТочка вставки блока:"))
  (setq p1 (getpoint p0 "\nУгол поворота :"))
...
(defun c:poz
	     (/	color_curr    ort    os	    sc	   key1	  key2	 key3	key4   key5   p1     p2
		p3     a1     a2     h	    p01	   w	  p02	 p03	p04    r      p05    a3
		old_err pi/2
	       )
  (setq pi/2 (/ pi 2.0))
  ...
  (progn
    (initget "STrelka SVarka SSVarka POint Nothing")
    (setq key1 (getkword "\nSTrelka/SVarka/SSVarka/POint/<Nothing>:"))
  )
  (progn
    (initget "Kontur Nothing")
    (setq key2 (getkword "\nKontur/<Nothing>:"))
  )
  (progn
    (initget "Montaz Nothing")
    (setq key3 (getkword "\nMontaz/<Nothing>:"))
  )
  (progn
    (initget "Kley Pajka Nothing")
    (setq key4 (getkword "\nKley/Pajka/<Nothing>:"))
  )
  (progn
    (initget "Nothing Circle KLejmo Polka")
...
;;;  (command "_.osnap" "_end")
  ...
  (cond
    ((= key1 "STrelka")
     (setq p01 (polar p1 a1 (* sc 5.)))
     (setq w (* sc 0.75))
     (command "_.pline" "_non" p1 "_h" "0" w "_non" p01 "")
    )
    ((= key1 "SVarka")
     (setq p01 (polar p1 a1 (* sc 5.)))
     (setq p02 (polar p01 (+ a1 1.4) (* sc 0.75)))
     (command "_.solid" "_non" p1 "_non" p01 "_non" p02 "" "")
    )
    ((= key1 "SSVarka")
     (setq p01 (polar p1 a1 (* sc 5.)))
     (setq p02 (polar p01 (- a1 1.4) (* sc 0.75)))
    ...
    ((= key1 "POint")
     (setq p01 (polar p1 a1 (* sc 0.5)))
     (setq p02 (polar p1 (+ a1 pi/2) (* sc 0.5)))
     (setq p03 (polar p1 (+ a1 pi) (* sc 0.5)))
     (setq p04 (polar p1 (- a1 pi/2) (* sc 0.5)))
  ...
  (cond
    ((= key4 "Kley")
     (setq p01 (polar p1 a1 (/ (distance p1 p2) 2.)))
  ...
  (cond
    ((= key5 "Polka")
     (command "_.line" "_non" p2 "_non" p3 "")
    )
    ((= key5 "Circle")
     (setq r (* sc 6.))
     (setvar "cecolor" "1")
