...
(defun c:for_prov  (/ ;
		    ...
		    fn ; Имя файла
		    f1 ; Дескриптор файла
		    ALFA_EPS ALFA_P   ALFA_TETA		A_ED	 A_EPS	  B	   BETA	    C
		    C_K	     DBX      DC       DELTA_BX	DELTA_C	 DELTA_K  DELTA_SIG	    DP
		    EL	     EPS      F1       G	I_NO	 LAMBDA_K LL_FOR_PROVER_SETUP
		    MU_EPS   MU_P     MU_TETA  N	NU	 PSI	  R	   RBX	    RC
		    RE_BX    RO	      STR      STR1	TETA	 _ALFA)
...
(defun for_prover:ac_tile  ()
  (mapcar (function (lambda (el) (action_tile (car el) "(for_prover-all_ac)")))
	  ll_for_prover))
