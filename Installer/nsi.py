...
def find_files(pth='.'):
  """
  Эта функция рекурсивно ищет файлы в дереве каталогов, начиная с
  указанного пути (pth). Она исключает каталоги .git и __pycache__, а
  абсолютные пути найденных файлов добавляются в глобальный список
  f_lst.
  """
  global f_lst
  f_lst=[]
...
def find_dirs(pth='.'):
  """
  Функция find_dirs рекурсивно находит все поддиректории в
  указанном каталоге. Она исключает каталоги .git и __pycache__, а
  абсолютные пути ко всем найденным поддиректориям добавляются в
  глобальный список d_lst.
  """
  global d_lst
  d_lst=[]
  for root, dirs, files in os.walk(pth):
...
def rm_dirs(f_inst, pth='.'):
  """
  Функция rm_dirs для каждой директории в списке d_lst, она
  записывает команду для удаления директории в файл f_inst.
  """
  global d_lst
  for d in d_lst:
...
def replace_slash_with_backslash(lst):
...
def create_nsi(pth='.'):
  ...
  #Поиск файлов 
  find_files(pth)
  f_lst=replace_slash_with_backslash(f_lst)
  f_lst.sort()
  f_lst.reverse()
  #Поиск каталогов
  find_dirs(pth)
...
VIAddVersionKey /LANG=${LANG_ENGLISH} "CompanyName" "Mykola Matvyeyev (MNASoft)"
