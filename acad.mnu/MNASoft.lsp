...
(load (strcat (MNAS_ArxPrj) "acad_w.sup/Utils/acad_sup_dir.lsp")
      "Message from M_utility.mnl Файл .../acad_w.sup/Utils/acad_sup_dir.lsp не найден."
)

(load (strcat (MNAS_ArxPrj_acad_w) "/opechatka/rus_cmd_opechatka.lsp")
      "Message from M_utility.mnl Файл .../acad_w.sup/opechatka/rus_cmd_opechatka.lsp не найден."
)
...
(defun mnasoft-vlx-loader (/		  abrev_cmd	 acad_pgp_cmd	add_prefix_cmd atoms_cmd
			   axis_cmd	  axis_podp_cmd	 big_fnt_cmd	change_cmd     circles_cmd
			   clock_cmd	  dims_cmd	 dim_style_cmd	dopusk_cmd     dxf_cmd
			   d_chprop_cmd	  edit_cmd	 entmake_cmd	exel_cmd       format_cmd
			   fors_rh_cmd	  for_lic_cmd	 f_otv_cmd	graf_cmd       hpgl_cmd
			   kompas_cmd	  layer_cmd	 lines_cmd	lopatka_cmd    man_cmd
			   pipe_cmd	  point_cmd	 pozition_cmd	razvertka_cmd  rename_cmd
			   rou_1_cmd	  rou_cmd	 scr_cmd	smesitel_vla   sort_shp_cmd
			   spec_cmd	  spring_cmd	 text_cmd	welding_cmd
			  )
  (setq
    ...
    change_cmd	   '("block_block" "block_point" "ch_wid" "point_block")
    ...
    graf_cmd	   '("graf")
    hpgl_cmd	   '("hpgl_in")
    kompas_cmd	   '("kompas_in" "kompas_out")
    layer_cmd	   '("bl_all_on"  "bl_all_th"  "bl_fr"	    "bl_off"	 "cl_sw_d"    "cl_s_d"
    ...
    lines_cmd	   '("d_chamf" "d_off" "d_otv" "d_rez" "exl" "xt" "xtcen" "cxt" "ltp" "r_off" "z0" "j_lns")
    lopatka_cmd	   '("lo")
    man_cmd	   '("man")
    pipe_cmd	   '("pipe")
    point_cmd	   '("pd" "xy" "xy_sc" "rot_p")
    pozition_cmd   '("edpos" "ep" "pozition_set_text_scale" "pos_export")
    razvertka_cmd  '("razvertka" "r_cone")
    rename_cmd	   '("ren")
    rou_cmd	   '("rou_i2")
    ...
    sort_shp_cmd   '("sort_shp")
    spec_cmd	   '("spec")
    Spring_cmd	   '("spring")
    text_cmd	   '("&A&B&C"	 "ate"	     "de"	 "l_text"    "stl"	 "te"	     "pos_export"
		     "te%%c"	 "te+"	     "te++"	 "te-"	     "te+kvsk"	 "te-kvsk"   "to_15"
		     "z_text"	 "x45%%d"    "te_*"	 "text_export"		 "te_<>_"
		    )
    welding_cmd	   '("edweld" "es" "ed-weld-txt" "est")
  )
  (autoload_cmds_and_op-cmds
    (strcat (MNAS_ArxPrj_vlx) "/MNASoft")
    (append abrev_cmd	    acad_pgp_cmd    add_prefix_cmd  atoms_cmd	    axis_cmd
	    axis_podp_cmd   big_fnt_cmd	    change_cmd	    circles_cmd	    clock_cmd
	    dims_cmd	    dim_style_cmd   dopusk_cmd	    dxf_cmd	    d_chprop_cmd
	    edit_cmd	    entmake_cmd	    exel_cmd	    format_cmd	    fors_rh_cmd
	    for_lic_cmd	    f_otv_cmd	    graf_cmd	    hpgl_cmd	    kompas_cmd
	    layer_cmd	    lines_cmd	    lopatka_cmd	    man_cmd	    pipe_cmd
	    point_cmd	    pozition_cmd    razvertka_cmd   rename_cmd	    rou_1_cmd
	    rou_cmd	    scr_cmd	    smesitel_vla    sort_shp_cmd    spec_cmd
	    spring_cmd	    text_cmd	    welding_cmd
	   )
  )
)
(mnasoft-vlx-loader)
